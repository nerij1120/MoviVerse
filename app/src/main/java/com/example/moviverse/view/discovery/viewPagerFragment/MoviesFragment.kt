...
import com.example.moviverse.databinding.FragmentMoviesBinding
import com.example.moviverse.viewmodel.DetailsViewModel
import com.example.moviverse.viewmodel.discovery.MoviesViewModel
import java.util.*
...
class MoviesFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        adapter = MovieSearchAdapter(ArrayList(), onClick = { movie, poster, title->
            detailsViewModel.getDetails(movie.id)
            val extras = FragmentNavigatorExtras(
                poster to "movie_poster_${movie.id}_1",
                title to "movie_title_${movie.id}_1",
            )

            findNavController().navigate(
                BottomNavDirections.discoveryToDetails(1),
                extras
            )
        }, 1)
