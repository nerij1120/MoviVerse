...
class FavoritesMoviesFragment : BaseFragment<FragmentFavoritesMoviesBinding>() {
    override val bindingInflater: (LayoutInflater, ViewGroup?, Boolean) -> FragmentFavoritesMoviesBinding
        get() = {
            layoutInflater, viewGroup, b ->
            FragmentFavoritesMoviesBinding.inflate(layoutInflater, viewGroup, b)
        }
    ...
    override fun onStart() {
        ...
        if(user != null){
            reference = FirebaseDatabase.getInstance().getReference("Favorites").child(user.uid).child("movies")
            loadData()
        }else{
        ...
        mListener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (moviesList.size == 0) {
                    binding.noMoviesLayout.visibility = View.VISIBLE
                    binding.noMoviesTxt.text = getString(R.string.no_movies_favorites)
                } else {
